...
async function getUserOrders(userLogin){
    ...
    orders.products = await getProducts().filter(product => productsIds.includes(product.id));
    return orders;
}
...
async function deleteOrder(userLogin, orderId) {
    ...
    users = users.map(user => {
        if (user.login === userLogin) {
            ...
            updatedOrders = updatedOrders.filter(userOrder => userOrder !== orderId);
            return { ...user, orders: updatedOrders };
        }
        return user;
    });
...
module.exports = {getOrders, getUserOrdersIds, getOrderById, createOrder, deleteOrder, getUserOrders }